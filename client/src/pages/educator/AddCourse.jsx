      ...
      <form onSubmit={handleSubmit} className='text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              placeholder='0'
              min={0}
              className='outline-none md:py-2.5 py-2 px-3 rounded w-28 border border-gray-500'
              required
            />
          </div>
        ...
        <div className='mt-4'>
          {chapters.map((chapter, chapterIndex) => (
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4  border-gray-400'>
              <div className='flex justify-between items-center p-4 border-b rounded border-gray-400'>
                <div className='flex items-center'>
                  <img
                    src={assets.dropdown_icon}
                    onClick={() => handleChapter("toggle", chapter.chapterId)}
                    alt='dropdown icon'
                    className={`mr-2 w-4 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`}
                  />
                ...
                <img src={assets.cross_icon} alt='cross icon' className='cursor-pointer' onClick={() => handleChapter("remove", chapter.chapterId)} />
              </div>
              {!chapter.collapsed && (
                <div className='p-4'>
          ...
          <div className='flex justify-center items-center bg-blue-100  mt-4 p-2 rounded-lg cursor-pointer' onClick={() => handleChapter("add")}>
            + Add Chapter
          </div>
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800/50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <button type='button' className='w-full bg-blue-400 text-white px-4 py-2 rounded cursor-pointer' onClick={addLecture}>
                  Add
                </button>

                <img src={assets.cross_icon} alt='cross' className='absolute top-4 right-4 w-4 cursor-pointer' onClick={() => setShowPopup(false)} />
              </div>
            </div>
          )}
        </div>
